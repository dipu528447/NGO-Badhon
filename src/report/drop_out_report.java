...
public class drop_out_report extends JPanel{
	BufferedImage image;
	...
	public drop_out_report(seasionbean s)
	{
		try {
			db.conect();
		...
		try{
			...
			if(product_category.equals("1"))
			{
				while(a<table.getRowCount())
				{
					...
					map.put("drop_out", table.getValueAt(a, 4));
				...
				JasperPrint jp=null;
				String input = "report/dropout_register.jrxml";
			...
			else if(product_category.equals("2"))
			{
				while(a<dps_table.getRowCount())
				{
					...
					map.put("drop_out", dps_table.getValueAt(a, 3));
				...
				JasperPrint jp=null;
				String input = "report/dps_dropout_register.jrxml";
				JasperReport com=JasperCompileManager.compileReport(input);
				jp = JasperFillManager.fillReport(com, map, new JRBeanCollectionDataSource(list));
				JasperViewer.viewReport(jp, false);
				list.clear();
...
//				}
//				r.close();
				ResultSet r=db.sta.executeQuery("select date,groupno,account_id from transaction where mendatory_savings_return+additional_savings_return!=0 && date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"' order by date asc)");
				while(r.next())
				{
			...
			else if(product_category.equals("2") && !from_date.getDate().equals("") && !to_date.getDate().equals(""))
			{
				...
				revalidate();
				ResultSet r=db.sta.executeQuery("select date,groupno,account_id from transaction where dps_return!=0 && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') && date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" order by date asc;");
				while(r.next())
				{
