...
public class loan_disburst_report extends JPanel{
	BufferedImage image;
...
//	
	...
	public loan_disburst_report(seasionbean s)
	{
		try {
			db.conect();
	...
	public void printevent(){
		try{
			...
			while(a<table.getRowCount())
			{
				...
				map.put("loan", table.getValueAt(a, 4));
			...
			JasperPrint jp=null;
			String input = "report/loan_disburst_report.jrxml";
	...
	public void searchevent()
	{
		try {
			if(!from_date.getDate().equals("") && !to_date.getDate().equals(""))
			{
				...
				ResultSet r=db.sta.executeQuery("select date,groupno,account_id,loan from transaction where loan!=0 && date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"')");
				while(r.next())
				{
					model.addRow(new Object[]{c.etob(r.getString("date")),c.etob(s.name(r.getString("groupno"), "group_info", "groupno")),c.etob(s.name(r.getString("account_id"), "member_info", "account")),s.name(r.getString("account_id"), "member_info", "name_ban"),c.etob(r.getString("loan"))});
				}
				r.close();
