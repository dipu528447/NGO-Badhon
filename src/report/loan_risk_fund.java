...
public class loan_risk_fund extends JPanel{
	BufferedImage image;
...
//	
	...
	public loan_risk_fund(seasionbean s)
	{
		try {
			db.conect();
	...
	public void printevent(){
		try{
			...
			while(a<table.getRowCount())
			{
				...
				map.put("loan_fee", table.getValueAt(a, 4).toString());
			...
			JasperPrint jp=null;
			String input = "report/loan_risk_fund.jrxml";
	...
	public void searchevent()
	{
		try {
			if(!from_date.getDate().equals("") && !to_date.getDate().equals(""))
			{
				...
				ResultSet r=db.sta.executeQuery("select date,groupno,account_id,loan from transaction where loan!=0 && date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') order by date asc");
				while(r.next())
				{
					model.addRow(new Object[]{c.etob(r.getString("date")),c.etob(s.name(r.getString("groupno"), "group_info", "groupno")),c.etob(s.name(r.getString("account_id"), "member_info", "account")),s.name(r.getString("account_id"), "member_info", "name_ban"),c.etob(String.valueOf(Integer.parseInt(r.getString("loan"))/100))});
				}
				r.close();
