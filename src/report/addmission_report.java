...
public class addmission_report extends JPanel{
	BufferedImage image;
	...
	String col[]={"Date","Group No","Member No","Name","Total member"};
	Object row[][]={};
	...
	String dps_col[]={"Date","Account","Name","Savings","Year","Deadline","Interest","total"};
	Object dps_row[][]={};
	...
	public addmission_report(seasionbean s)
	{
		try {
			db.conect();
	...
	public void searchevent()
	{
		try {
			if(!from_date.getDate().equals("") && !to_date.getDate().equals("") && product_category.equals("1"))
			{
				...
				revalidate();
				ResultSet r=db.sta.executeQuery("select addmission_date,groupno,account,name_ban from member_info where addmission_date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && addmission_date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"')  order by addmission_date asc");
				while(r.next())
				{
			...
			else if(product_category.equals("2") && !from_date.getDate().equals("") && !to_date.getDate().equals(""))
			{
				...
				revalidate();
				ResultSet r=db.sta.executeQuery("select addmission_date,groupno,account,name_ban,dps_category,dps_year from member_info where product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') && addmission_date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && addmission_date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && activition='y' order by addmission_date");
				while(r.next())
				{
					
					int year=Integer.parseInt(r.getString("dps_year"))+Integer.parseInt(r.getString("addmission_date").substring(0,4));
					int month=Integer.parseInt(r.getString("addmission_date").substring(5,7));
					int day=Integer.parseInt(r.getString("addmission_date").substring(8,10));
					int interest=((Integer.parseInt(r.getString("dps_category"))*Integer.parseInt(r.getString("dps_year"))*12)*9)/100;
					int total=Integer.parseInt(r.getString("dps_category"))*(Integer.parseInt(r.getString("dps_year")))*12;
					...
					dps_model.addRow(new Object[]{c.etob(r.getString("addmission_date")),c.etob("02"+admit+groupno+account),r.getString("name_ban"),c.etob(r.getString("dps_category")),c.etob(r.getString("dps_year")),c.etob(year+"-"+month+"-"+day),c.etob(String.valueOf(interest)),c.etob(String.valueOf(total+interest))});
				}
				dps_model.addRow(new Object[]{"","মোট",c.etob(String.valueOf(dps_table.getRowCount()))+"জন",c.etob(String.valueOf(dpstotal(3))),"","",c.etob(String.valueOf(dpstotal(6))),c.etob(String.valueOf(dpstotal(7)))});
