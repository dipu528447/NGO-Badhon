...
public class passbook_report extends JPanel{
	BufferedImage image;
	...
	public passbook_report(seasionbean s)
	{
		try {
			db.conect();
		...
		try{
			...
			if(product_category.equals("1"))
			{
				while(a<table.getRowCount())
				{
					...
					map.put("passbook", table.getValueAt(a, 4));
				...
				JasperPrint jp=null;
				String input = "report/passbook_register.jrxml";
			...
			else if(product_category.equals("2"))
			{
				while(a<dps_table.getRowCount())
				{
					...
					map.put("passbook", dps_table.getValueAt(a, 3));
				...
				JasperPrint jp=null;
				String input = "report/dps_passbook_report.jrxml";
				JasperReport com=JasperCompileManager.compileReport(input);
				jp = JasperFillManager.fillReport(com, map, new JRBeanCollectionDataSource(list));
				JasperViewer.viewReport(jp, false);
				list.clear();
	...
	public void searchevent()
	{
		try {
			if(!from_date.getDate().equals("") && !to_date.getDate().equals("") && product_category.equals("1"))
			{
				...
				revalidate();
				ResultSet r=db.sta.executeQuery("select addmission_date,groupno,account,name_ban from member_info where addmission_date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && addmission_date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') && activition='y' order by addmission_date asc");
				while(r.next())
				{
				...
				r.close();
				r=db.sta.executeQuery("select date,groupno,account_id from transaction where duplicate_passbook!=0 && date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') order by date asc");
				while(r.next())
				{
					model.addRow(new Object[]{c.etob(r.getString("date")),c.etob(s.name(r.getString("groupno"), "group_info", "groupno")),c.etob(s.name(r.getString("account_id"), "member_info", "account")),s.name(r.getString("account_id"), "member_info", "name_ban"),c.etob("1")});
				}
				r.close();
				model.addRow(new Object[]{"","","","মোট",c.etob(String.valueOf(total(4)))});
			...
			else if(product_category.equals("2") && !from_date.getDate().equals("") && !to_date.getDate().equals(""))
			{
				...
				revalidate();
				ResultSet r=db.sta.executeQuery("select addmission_date,groupno,account,name_ban from member_info where product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') && addmission_date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && addmission_date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+" && activition='y'");
				while(r.next())
				{
					String admit=r.getString("addmission_date").toString().substring(0,4)+r.getString("addmission_date").toString().substring(5,7);
					...
					dps_model.addRow(new Object[]{c.etob(r.getString("addmission_date")),c.etob("02"+admit+groupno+account),r.getString("name_ban"),c.etob("1")});
				}
				r.close();
				r=db.sta.executeQuery("select date,groupno,account_id from transaction where dps_duplicate_passbook!=0 && product=(select id from product where product_name='"+cmbproduct.getSelectedItem().toString()+"') && date>="+new SimpleDateFormat("YYYYMMdd").format(from_date.getDate())+" && date<="+new SimpleDateFormat("YYYYMMdd").format(to_date.getDate())+";");
				while(r.next())
				{
					...
					dps_model.addRow(new Object[]{c.etob(r.getString("date")),c.etob("02"+admit+groupno+account),s.name(r.getString("account_id"), "member_info", "name_ban"),c.etob("1")});
				}
				r.close();
